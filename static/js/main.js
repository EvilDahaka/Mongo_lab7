            ...
            <div class="col-md-6 mb-4">
                <div class="card post-card h-100">
                    <div class="card-body">
                        ...
                        <div class="mb-2">
                            ${post.tags.map(t => `<span class="badge bg-secondary badge-tag" onclick="filterByTag('${t}')" title="Клікніть, щоб побачити всі пости з цим тегом">${t}</span>`).join('')}
                        </div>
...
async function searchPosts() {
    ...
    displayPosts(posts, `Результати пошуку: "${query}"`);
}
...
async function filterByTag(tag) {
    try {
        const posts = await apiCall(`/api/posts/tag/${encodeURIComponent(tag)}`);
        displayPosts(posts, `Пости з тегом: "${tag}"`);
    } catch (error) {
        console.error('Error filtering by tag:', error);
        alert('Помилка фільтрації по тегу');
    }
}

function displayPosts(posts, title = null) {
    let html = '';
        ...
        html += `
            <div class="col-12">
                <div class="alert alert-info text-center">
                    <i class="bi bi-info-circle"></i> Постів не знайдено
                </div>
            </div>
        `;
    }
...
async function showStats() {
    ...
    let html = `
        ...
        return `
                                        <div class="list-group-item" style="cursor: pointer;" onclick="filterByTag('${tag.tag}')">
